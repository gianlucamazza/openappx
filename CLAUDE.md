...
- **Non-ASCII entry names need flag bit 11** (`0x800`), set per entry. Without it a reader decodes the name as CP437; `zipfile` writes it for you, our hand-rolled writer must not forget.
- **ZIP64 is required at the EOCD and forbidden on the records.** Appx wants the ZIP64 end-of-central-directory (without it: `0x8007000B`), but a package whose entries carry ZIP64 *extra fields* is refused with the same code — both measured on a device, and standard ZIP readers accept either, so only hardware shows it. That is why `pack` refuses files above 4 GiB rather than encoding them.
